...
contract('GenesisNFT', async function(accounts) {
    ...
    it('Guy 1 buying for exact value, guy 2 paying too much and being refunded', async () => {
        let genesisNFT = await GenesisNFT.new(name, symbol, benficiary, basePrice, multiplier, divisor, limit, { from: creator } );
        ...
        assert.closeTo(guy1BalanceAfter, guy1BalanceBefore + 0.1, 0.001, "guy 1 balance should be reduced");
        ...
        assert.closeTo(guy2BalanceAfter, guy2BalanceBefore + 0.11, 0.001, "guy 2 balance should be refunded");
    ...
    it('Guy 1 2 3 buying, guy 4 being rejected', async () => {
        ...
        assert.equal(beneficiaryBalanceBefore + 0.1 + 0.11 + 0.121, beneficiaryBalanceAfter);

        // ALSO SOME SANITY CHECK IF THE ERC721 is working fine (sanity check, not comprehensive)
        await genesisNFT.safeTransferFrom(guy3, guy2, 2, from: guy4);
